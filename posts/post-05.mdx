...
`e.stopPropagation()` 을 통해 이벤트 전파를 막을 수 있습니다. 단 버블링 단계에서만 전파를 막을 수 있고 캡처링 단계는 막을 수 없습니다. `<p>` 태그를 클릭 시 콘솔 로그에는 p만 찍힙니다. 하지만 `<div>`를 클릭 시 전파되어 form 태그도 콘솔에 찍히는 걸 확인할 수 있습니다.
...
`<p>` 태그를 클릭했지만 콘솔에는 `form` > `p` > `div` 가 로그에 찍히는 것을 볼 수 있습니다. 그 이유는 form 태그의 핸들러는 캡처링 단계에서 이벤트를 캐치하고 `p`와 `div` 태그는 버블링 단계에서 이벤트를 캐치하니 `form`부터 이벤트 핸들러가 실행되기 때문입니다.

## 이벤트 위임

자바스크립트는 기본적으로 버블링 단계에서 이벤트를 캐치하는데요. 이 흐름을 이용하여 모든 요소에 이벤트를 등록하지 않고 상위 요소에 이벤트를 등록하여 이벤트를 캐치할 수 있습니다.
