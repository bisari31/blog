...
(자바스크립트 코드로 해결할 수 있지만 css에서도 속성을 제공하기 때문에 굳이 자바스크립트를 사용할 이유가 없다. css로 문제를 해결할 수 있다면 성능 측면에서 이점을 가져갈 수 있다.)  
...
여러 줄을 노출 시킬땐 `-webkit-line-clamp` 속성을 원하는 줄 수에 맞게 설정하면 된다.
