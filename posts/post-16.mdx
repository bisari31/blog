...
title: Sass 공식 문서 해석
date: 2023-10-19
description: Sass에서 use와 import의 차이, 그리고 mixin과 extend의 차이 등 Scss 기능들에 대해 살펴보겠습니다.
...
`@use`를 통해 불러온 모듈은 확장자명을 생략할 수 있습니다. `@use 'variables'`는 `variables.scss`, `variables.sass`, `variables.css`를 자동으로 불러옵니다. 해당 모듈을 사용하기 위해선 네임스페이스를 앞에 사용해야 합니다. 임의의 이름으로 변경도 가능합니다. `as *`을 통해 네임스페이스를 생략할 수도 있지만 다른 멤버(mixin, function, 변수 등)와 충돌 위험이 있기 때문에 본인이 작성한 스타일시트에 한해서 사용해야 합니다.
...
@import 규칙은 모든 것이 전역적이기 때문에 각 파일에서 참조하는 멤버의 출처를 알기 어려우며, 이름 충돌이 일어날 확률이 매우 높기에 Sass 팀은 @import 규칙의 사용을 권장하지 않습니다. 해당 규칙은 향후 폐지할 계획이라고 공식 문서에는 설명하고 있습니다.
...
@return은 @function 내에서만 사용 가능하며 @function은 @return으로 끝나야 합니다. return이 발생되면 즉시 함수를 종료하고 결과를 반환합니다.

### @extend

페이지를 디자인할 때 해당 클래스의 고유한 스타일이 모두 필요한 경우가 있습니다.
...
@extend는 해당 클래스의 모든 스타일을 확장하게 됩니다. `:hover`와 같이 CSS 의사 클래스의 경우도 이에 해당됩니다.
...
확장만을 목적으로 하는 스타일을 생성할 때 자리 유용하게 사용할 수 있습니다. 해당 선택자는 css에 컴파일되지 않지만 확장한 클래스에는 포함됩니다.
